...
void read_graph_from_file2(char *filename, int *N, int **row_ptr, int **col_idx, double **val) {
    ...
    while (line_count < edges && fgets(line, sizeof(line), file)) {
        if (line[0] != '#') {
            int from, to;
            if (sscanf(line, "%d %d", &from, &to) == 2) {
                if (from >= 0 && from < *N && to >= 0 && to < *N) {
                    edge_list[line_count].from = from;
                    edge_list[line_count].to = to;
                    line_count++;
                } 
    ...
    *col_idx = malloc(line_count * sizeof(int));
    *val = malloc(line_count * sizeof(double));
